...
/** Texto corrido: fixo, independente da cor de detalhe. */
const TINTA = "#2f3542";
const SUAVE = "#57606f";
const CLARO = "#a4b0be";

function Secao({ titulo, linha, semLinha = false, children }: { titulo: string; linha: string; semLinha?: boolean; children: ReactNode }) {
  return (
...
export default function ModeloMinimalista({ d, cor = COR_PADRAO.minimalista }: { d: DadosCurriculo; cor?: string }) {
  const { destaque: DESTAQUE, faixa: FAIXA, fotoFundo, linha: LINHA } = coresMinimalista(cor);
  const contatos = [
  ...
  return (
    <article className="cv-pagina" style={{ color: TINTA }}>
      <header className="px-[13mm] py-[9mm] flex items-center gap-[8mm]" style={{ backgroundColor: FAIXA }}>
        <FotoCV src={d.foto} iniciais={d.iniciais} forma="quadrada" tamanho="30mm" raio="3mm" fundo={fotoFundo} cor={DESTAQUE} />
      ...
      <main className="px-[13mm] pb-[14mm]">
            ...
            <p className="mt-[7mm] text-[10pt] flex flex-wrap items-center gap-y-[1mm]" style={{ color: SUAVE }}>
              {contatos.map((c, i) => (
                <span key={i} className="inline-flex items-center gap-[1.5mm]">
                  ...
                  <span style={{ color: DESTAQUE }}>{c.icone}</span>
                  {c.texto}
                </span>
              ))}
            </p>
        ...
        {d.resumo && (
          <Secao titulo="SOBRE MIM" linha={LINHA} semLinha>
        ...
        {d.experiencias.length > 0 && (
          <Secao titulo="EXPERIÊNCIA PROFISSIONAL" linha={LINHA}>
        ...
        {d.formacao.length > 0 && (
          <Secao titulo="EDUCAÇÃO" linha={LINHA}>
        ...
        {d.habilidades.length > 0 && (
          <Secao titulo="COMPETÊNCIAS" linha={LINHA}>
        ...
        {(d.idiomas.length > 0 || d.disponibilidade) && (
          <Secao titulo="MAIS INFORMAÇÕES" linha={LINHA}>
